...
| Student | `/api/v1/student/dashboard`, `/schedule`, `/plan`, `/analytics`, `/assignments`, `/assignments/{assignmentId}`, `/tasks/{taskId}`, `/tasks/{taskId}/attempts`, `/progress` | Student-safe serializers omit rationale, change summaries, teacher notes, answers, solutions and local/internal source paths. |
| Teacher | `/api/v1/teacher/dashboard`, `/students`, `/students/{studentId}`, `/students/{studentId}/plan`, `/students/{studentId}/plan/publish`, `/students/{studentId}/plan/history`, `/students/{studentId}/plan/feedback-preview`, `/students/{studentId}/plan/adjustments/{adjustmentId}/apply`, `/students/{studentId}/schedule`, `/students/{studentId}/assignments`, `/students/{studentId}/analytics`, `/task-bank`, `/tasks/{taskId}`, `/tasks/bulk` | Teacher routes require server-side teacher/owner/tutor role checks and ownership checks where a student is addressed. Plan editing changes only a draft; publishing creates an immutable active version, and deterministic feedback proposals are applied only by an explicit teacher action. |
| Imports | `/api/v1/teacher/imports`, `/imports/{importId}`, `/upload`, `/analyze`, `/rows`, `/rows/{rowId}`, `/apply` | Upload/URL sources are stored privately, analyzed without changing the bank, reviewed row by row and applied only after explicit confirmation. |
